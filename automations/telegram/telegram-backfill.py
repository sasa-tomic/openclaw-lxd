...
class BackfillProgress(JsonStateFile):
    """Track backfill progress for resumability."""

    def __init__(self, path: Path):
        path.parent.mkdir(parents=True, exist_ok=True)
        super().__init__(
            path, default={"chats": {}, "started": None, "completed": None}
        )
    ...
    def update_chat(
        self, chat_id: int, name: str, offset_id: int, total: int, done: bool = False
    ) -> None:
...
class TelegramBackfill:
    ...
    async def backfill_chat(self, dialog, chat_idx: int, total_chats: int) -> int:
        ...
        if self.progress.is_chat_done(chat_id) and not self.force:
            print(
                f"  [{chat_idx}/{total_chats}] {chat_name}: Already backfilled, skipping"
            )
            return 0
        ...
        if oldest_existing_ts:
            print(
                f"oldest existing: {oldest_existing_ts.strftime('%Y-%m-%d')}",
                end=" ",
                flush=True,
            )
        ...
        try:
            async for msg in self.client.iter_messages(
                ...
                if self._shutdown:
                    ...
                    if all_messages:
                        self.progress.update_chat(
                            chat_id, chat_name, msg.id, len(all_messages), done=False
                        )
                    return len(all_messages)

                if not isinstance(msg, TelegramMessage) or (
                    not msg.text and not msg.media
                ):
                    continue
                ...
                if (
                    oldest_existing_ts
                    and msg.date.replace(tzinfo=timezone.utc) >= oldest_existing_ts
                ):
                    continue
            ...
            if all_messages:
                ...
                self.progress.update_chat(
                    chat_id, chat_name, last_msg.id, len(all_messages), done=False
                )
        ...
        # Mark as done
        self.progress.update_chat(
            chat_id, chat_name, 0, len(formatted_messages), done=True
        )
    ...
    async def run(self):
        ...
        if self.target_chat:
            ...
            dialogs = [
                d for d in dialogs if target_lower in get_entity_name(d.entity).lower()
            ]
            if not dialogs:
                print(
                    f"ERROR: No chat found matching '{self.target_chat}'",
                    file=sys.stderr,
                )
                return 1
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Backfill Telegram history to Obsidian"
    )
    parser.add_argument(
        "--chat", help="Only backfill specific chat (partial name match)"
    )
    parser.add_argument(
        "--force", action="store_true", help="Re-backfill even if already done"
    )
    parser.add_argument(
        "--status", action="store_true", help="Show backfill progress status"
    )
    ...
    if args.status:
        ...
        for chat_id, info in progress._data.get("chats", {}).items():
            ...
            print(
                f"  {status} {info.get('name', chat_id)}: {info.get('total_fetched', 0)} messages"
            )
        return 0
